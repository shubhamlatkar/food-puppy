...
import com.foodgrid.delivery.command.internal.event.broker.AuthenticationEventBroker;
...
class AuthenticationEventBrokerTests {
    ...
    @Test
    @DisplayName("Tests send method of AuthenticationEventBroker")
    void send() {
        when(userSession.getUserId()).thenReturn("1");
        var tempRole = new Role("USER", List.of(new Authority("1", "TEST_AUTH")));
        var tempUser = new User("test_username", "1234567890", "testemail@email.com", "test_pass", List.of(tempRole), UserTypes.DELIVERY);
