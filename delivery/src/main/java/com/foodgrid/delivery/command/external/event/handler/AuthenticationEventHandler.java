package com.foodgrid.delivery.command.external.event.handler;
...
public class AuthenticationEventHandler {
    ...
    public void authConsumer(AuthenticationEvent event) {
        event.setUsers(
                event.getUsers()
                        .stream()
                        .filter(userAuthEventDTO -> Boolean.FALSE.equals(userAuthEventDTO.getUserType().equals(UserTypes.DELIVERY)))
