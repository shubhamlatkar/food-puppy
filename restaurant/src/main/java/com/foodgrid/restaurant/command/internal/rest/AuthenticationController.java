...
import com.foodgrid.restaurant.command.internal.payload.dto.request.RestaurantSignUp;
...
public class AuthenticationController {
    ...
    @Autowired
    public AuthenticationController(AuthenticationService authenticationService, RestaurantAuthenticationService restaurantAuthenticationService) {
        this.authenticationService = authenticationService;
        this.restaurantAuthenticationService = restaurantAuthenticationService;
    }

    @PutMapping("/${endpoint.authentication.signup}")
    public ResponseEntity<AuthenticationActionResponse> signupUser(@Valid @RequestBody RestaurantSignUp signupRequest, BindingResult result) {
        return ResponseEntity.ok(authenticationService.signup(
                new SignUp(
                        signupRequest.getUsername(),
                        signupRequest.getEmail(),
                        new HashSet<>(Collections.singletonList("ROLE_RESTAURANT")),
    ...
    public ResponseEntity<GenericIdResponse> deleteMe() {
        return ResponseEntity.ok(restaurantAuthenticationService.deleteMe());
    }
}