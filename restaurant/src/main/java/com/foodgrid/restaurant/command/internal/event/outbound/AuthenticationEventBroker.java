...
public class AuthenticationEventBroker {
    ...
    public void send() {
        ...
        users.forEach(user -> {
            if (user.getMetadata().getLastUpdatedAt().getTime() > start.getTime() && user.getType().equals(UserTypes.RESTAURANT))
        ...
        if (deletedUsers.getUsers() != null && Boolean.FALSE.equals(deletedUsers.getUsers().isEmpty())) {
            deletedUsers.getUsers().forEach(user1 -> userList.add(new UserToUserAuthEvent(user1, UserTypes.RESTAURANT).getUser()));
